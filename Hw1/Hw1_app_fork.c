// to make 7seg and led run simultaneously
...
void write_seg_device(int total_cost){
    ...
    if (pid == 0){
        ...
        waitpid(pid, NULL, WNOHANG); // do not wait for child process, parent will go on
    }
}
...
void write_led_device(int distance){
    ...
    if (pid == 0){
        ...
        if (fd == -1) {
            perror("Failed to open the device");
            exit(EXIT_FAILURE); // Return instead of EXIT_FAILURE
        }
